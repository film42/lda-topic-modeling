...
class LDA:
    def __init__(self, documents, alpha, beta, n_topics):
        ...
        self.t_assignments = np.zeros((self.documents, self.vocab_size))
        ...
        self.n_words_vec = np.zeros(self.vocab_size)

        # Init
        for m in xrange(self.n_documents):
            # i is a number between 0 and doc_length-1
            # w is a number between 0 and vocab_size-1
            for i, w in enumerate(word_indices(self.documents[m, :])):
                # choose an arbitrary topic as first topic for word i
                z = np.random.randint(self.n_topics)
                self.t_docs_count_matrix[m, z] += 1
                self.n_documents_vec[m] += 1
                self.t_word_count_matrix[z, w] += 1
                self.n_words_vec[z] += 1
                self.t_assignments[m, i] = z

    def sample(self, max_iterations=500):
        # Initialize the z vector
        z = [np.random.randint(self.n_topics) for _ in xrange(self.vocab_size)]
        # Iterations
    ...
    def create_pi_vector(self, z, w, d):
        """
        ...
        """
        new_pi_vector = []
        left = (self.t_word_count_matrix[:, w] + self.beta) / (self.nz + self.beta * self.vocab_size)
        right = (self.t_docs_count_matrix[d, :] + self.alpha) / (self.nm[d] + self.alpha * self.n_topics)
    ...
    def log_likelihood(self):
        ...
        for m in xrange(self.n_documents):
            lik += log_multi_beta(self.t_docs_count_matrix[m, :] + self.alpha)
...
if __name__ == "__main__":
    ...
    for it, phi in enumerate(sampler.sample(max_iterations=10)):
    ...
    for index, topic in enumerate(sampler.t_assignments):
